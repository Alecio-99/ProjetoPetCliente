...
public class ServicePlanos {
    ...
    //metodo para cadastro dos clientes que nossos clientes vão ter
   public EntityCliente cadastraCliente(ClienteDTO clienteDTO){
        Optional<EntityCliente> cliente = clienteRepository.findByCpf(clienteDTO.cpf());
        if(cliente.isPresent()){
            System.out.println("Você já possui cadastro");
            throw new RuntimeException("Você já possui cadastro");
        }
        EntityCliente cadastra = new EntityCliente();
        cadastra.setNome(clienteDTO.nome());
        cadastra.setEmail(clienteDTO.email());
        cadastra.setCpf(clienteDTO.cpf());
        cadastra.setTelefone(clienteDTO.telefone());
        cadastra.setCep(clienteDTO.cep());
        cadastra.setEndereco(clienteDTO.endereco());
        cadastra.setPassword(clienteDTO.password());

        return clienteRepository.save(cadastra);
